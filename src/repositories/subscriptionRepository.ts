...
export async function findByUnsubToken(
  ...
  const result = await pool.query<Subscription>(
    "SELECT * FROM subscriptions WHERE unsub_token = $1",
    [token],
  );
  return result.rows[0] ?? null;
}
...
export async function findConfirmedByEmail(
  email: string,
...
export async function findAllDistinctReposConfirmed(): Promise<
...
export async function findConfirmedSubscribersByRepo(params: {
  ...
  { email: string; unsub_token: string; last_seen_tag: string | null }[]
> {
  const result = await pool.query(
...
export async function updateLastSeenTag(params: {
  owner: string;
  repo: string;
  tag: string;
}): Promise<void> {
  await pool.query(
    "UPDATE subscriptions SET last_seen_tag = $3 WHERE owner = $1 AND repo = $2",
