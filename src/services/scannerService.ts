...
export async function scan(): Promise<void> {
  ...
  for (const { owner, repo } of repos) {
    try {
      ...
      const subscribers =
        await subscriptionRepository.findConfirmedSubscribersByRepo({
          owner,
          repo,
        });
      ...
      if (err instanceof RateLimitError) {
        const retryTime = err.retryAfter
          ? new Date(err.retryAfter * 1000).toISOString()
          : "unknown";
        console.warn(
          `[scanner] GitHub rate limit hit. Retry after: ${retryTime}. Aborting scan.`,
        );
        return;
      }
